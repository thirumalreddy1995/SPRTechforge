  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-2">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border font-semibold text-sm transition-all ${showMetrics ? 'bg-slate-800 text-white border-slate-800 shadow-md' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              {showMetrics
            ...
            {showMetrics ? 'Hide Financials' : 'View All Financials'}
          </button>
          {showMetrics && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title="Candidate Overview" className="lg:col-span-2" action={<Link to="/candidates" className="text-xs text-spr-600 font-bold hover:underline">All Candidates</Link>}>
        ...
        {showMetrics && <Card title="Ledger Summary" action={<Link to="/finance/accounts" className="text-xs text-spr-600 font-bold hover:underline">View Accounts</Link>}>
          <div className="space-y-3 pt-1">
      ...
      {/* P&L + Recent Transactions — shown only when metrics are visible */}
      {showMetrics && <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* Interview Pipeline + Batch Overview — always visible */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card title="Active Interview Schedule" action={<Link to="/training/interviews" className="text-xs text-spr-600 font-bold hover:underline">View All →</Link>}>
          <div className="space-y-2 pt-1">
            {upcomingInterviews.length > 0 ? upcomingInterviews.map(inv => {
              const c = candidates.find(x => x.id === inv.candidateId);
              const isToday = inv.date === todayStr;
              return (
                <Link key={inv.id} to="/training/interviews"
                  className={`flex justify-between items-center p-3 rounded-lg border transition-colors cursor-pointer ${isToday ? 'bg-purple-50 border-purple-100 hover:bg-purple-100' : 'bg-gray-50 border-gray-100 hover:bg-blue-50 hover:border-blue-100'}`}>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-gray-900 text-sm truncate">{c?.name || 'Unknown'}</p>
                    <p className="text-[10px] text-gray-500 truncate">{inv.companyName} · {new Date(inv.date).toLocaleDateString()} · {inv.time}</p>
                  </div>
        ...
        <Card title="Batch Overview" action={<Link to="/candidates" className="text-xs text-spr-600 font-bold hover:underline">All Candidates</Link>}>
          <div className="space-y-3 pt-1">
            ...
            {candidates.length > 6 && <p className="text-xs text-gray-400 text-center pt-1">+ {candidates.length - 6} more candidates</p>}
            {candidates.length === 0 && <div className="text-center py-6 text-gray-400 text-sm">No candidates enrolled yet.</div>}
          </div>
        </Card>
      </div>
    </div>
