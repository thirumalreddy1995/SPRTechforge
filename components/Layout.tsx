...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { user, logout, interviews, candidates, webLeads, chatMessages, chats, meetings } = useApp();
  ...
  const todayMeetingsCount = meetings.filter(m =>
    m.status === 'scheduled' &&
    new Date(m.startTime).getTime() >= startOfToday.getTime() &&
    new Date(m.startTime).getTime() < startOfTomorrow.getTime() &&
          ...
          {user.role !== 'candidate' ? (
            <>
              ...
              >
                ...
                {!isSidebarCollapsed ? (
                  <Link
                    to="/meetings"
                    onClick={closeMobile}
                    className={`flex items-center justify-between gap-2 px-4 py-2.5 rounded-lg transition-all duration-200 mb-0.5 font-medium text-sm overflow-hidden whitespace-nowrap ${location.pathname.startsWith('/meetings') ? 'bg-blue-600/20 text-blue-300' : 'text-slate-400 hover:bg-slate-800 hover:text-slate-100'}`}
                  >
                    <span>Meetings</span>
                    {todayMeetingsCount > 0 && (
                      <span className="bg-purple-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] text-center shrink-0">
                        Today: {todayMeetingsCount}
                      </span>
                    )}
                  </Link>
                ) : (
                ...
                {(user.role === 'admin' || user.modules.includes('users')) && (
                  <NavItem to="/events/manage" label="Events" onClick={closeMobile} isSidebarCollapsed={isSidebarCollapsed} setIsSidebarCollapsed={setIsSidebarCollapsed} />
                )}
              </CollapsibleGroup>
              ...
              >
                ...
                <NavItem to="/training/interview-prep" label="Prompt Practice" onClick={closeMobile} isSidebarCollapsed={isSidebarCollapsed} setIsSidebarCollapsed={setIsSidebarCollapsed} />
            ...
            <>
              ...
              <NavItem isSidebarCollapsed={isSidebarCollapsed} setIsSidebarCollapsed={setIsSidebarCollapsed} to="/meetings" icon={<svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>} label="Meetings" onClick={closeMobile} />
